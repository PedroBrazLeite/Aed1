...
int remove_pos(Lista *lst, int pos, char *retorno)
{
    ...
    while (pos--)
        aux = aux->prox; // percorre a lista ate a posicao desejada
    if (aux->prox == NULL)
        return 0; // caso n encontre, retorna erro
...
int obtem_valor_elem(Lista *lst, int index, char *result)
{
    ...
    while (aux != NULL && index--)
        aux = aux->prox; // percorre ate encontrar
    if (aux == NULL)
        return 0;
...
int listas_iguais(Lista *lst1, Lista *lst2)
{
    ...
    while (tam--)
    { // percorre verificando se sao iguais
