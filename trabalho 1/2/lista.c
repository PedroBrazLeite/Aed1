...
{ // como eh uma lista ordenada, todo insere precisa ser ordenado.
...
int remove_impares(Lista list)
{
    ...
    { // passa por toda a lista
        if (list->lista[i] & 1)
        { // se o numero for impar, remove (bitwise operation)
            for (int j = i; j < list->fim; j++)
            { // Remove fazendo shift
                if (j + 1 != max)
...
int menor(Lista list)
{
    ...
    { // percorre a lista em busca do menor numero
